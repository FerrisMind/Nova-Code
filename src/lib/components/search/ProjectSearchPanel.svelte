<script lang="ts">
  ...
  import { projectSearchStore } from '$lib/stores/searchStore';
  import { editorStore } from '$lib/stores/editorStore';
  import { workspaceStore } from '$lib/stores/workspaceStore';

  // Reactive state
  const state = $derived($projectSearchStore);
  const resultCount = $derived(state.results.length);
  const isSearching = $derived(state.isSearching);
...
<div class="project-search-panel">
  ...
  <div class="search-controls">
    <div class="input-row">
      ...
      {#if isSearching}
        <button class="action-btn cancel-btn" onclick={handleCancel} title="Cancel search">
          Cancel
        </button>
      {:else if resultCount > 0}
        <button class="action-btn clear-btn" onclick={handleClear} title="Clear results">
          Clear
        </button>
      {/if}
    </div>
  ...
  {:else if resultCount > 0}
    <div class="search-status">
      <span class="status-text">{resultCount} result{resultCount === 1 ? '' : 's'}</span>
    </div>
  {/if}
